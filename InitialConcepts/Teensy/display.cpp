...
// ------------------------------------------------------------
// Buttons — debounced edge detect
// ------------------------------------------------------------
struct Btn {
  uint8_t pin;
  bool    last = true;      // pullup idle HIGH
  uint32_t t_change = 0;
  bool pressed(uint32_t now) {
    bool v = digitalRead(pin);
    if (v != last && now - t_change > 40) {
      t_change = now;
      last = v;
      return !v;            // falling edge = press
    }
    return false;
  }
};
static Btn s_btn_tab{PIN_BTN_TAB}, s_btn_up{PIN_BTN_UP},
           s_btn_dn{PIN_BTN_DOWN}, s_btn_sel{PIN_BTN_SEL};
...
// ------------------------------------------------------------
// Button + touch handling (called every loop pass)
// ------------------------------------------------------------
void display_buttons(NvState& s, uint32_t now, bool* dirty) {
  int16_t tx, ty;
  if (touch_tap(now, &tx, &ty)) touch_dispatch(s, tx, ty, dirty);

  if (s_btn_tab.pressed(now))
    goto_tab((Tab)((s_tab + 1) % TAB_COUNT));

  bool up = s_btn_up.pressed(now);
  bool dn = s_btn_dn.pressed(now);
  bool sel = s_btn_sel.pressed(now);
  if (!(up || dn || sel)) return;

  switch (s_tab) {
    case TAB_OPER:
      if (sel) activate(s, dirty);
      if ((up || dn) && !s.autorange)
        measure_set_range(s, s.range == RANGE_20MV ? RANGE_200MV : RANGE_20MV);
      break;
    case TAB_SET:
      if (up) s_cursor = (s_cursor + SR_COUNT - 1) % SR_COUNT;
      if (dn) s_cursor = (s_cursor + 1) % SR_COUNT;
      if (sel) activate(s, dirty);
      break;
    case TAB_CAL:
      if (up) s_cursor = (s_cursor + CR_ACTIONS - 1) % CR_ACTIONS;
      if (dn) s_cursor = (s_cursor + 1) % CR_ACTIONS;
      if (sel) activate(s, dirty);
      break;
    default: break;
  }
...
void display_init(Adafruit_ILI9341& tft) {
  pinMode(PIN_BTN_TAB, INPUT_PULLUP);
