/* Nanovoltmeter Controller — Teensy 4.1
   ...
   Architecture (mirrors the dual-PDH controller):
     ...
     display.cpp    ILI9341 UI: Operation / Settings / Calibration
                    tabs, XPT2046 touchscreen (no physical buttons)
...
void setup() {
  ...
  display_init(tft);
  display_backlight(g_nv.backlight);       // restore persisted brightness
  measure_init(g_nv);
...
void loop() {
  ...
  relays_tick(now);
  display_input(g_nv, now, &g_flags.settings_dirty);
